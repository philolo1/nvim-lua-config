-- package manager
require("philolo1.lazy")


-- general settings
require("philolo1.remap")
require("philolo1.command")
require("philolo1.set")


-- plugins
require("philolo1.telescope")
require("philolo1.colors")
require("philolo1.treesitter")
require("philolo1.comment");
require("philolo1.harpoon")
require("philolo1.fugitive")
require("philolo1.undotree")
require("philolo1.lualine")
require("philolo1.rust-tools")
require("philolo1.nvterm")

-- lsp
require("philolo1.lsp")
require("philolo1.lsp-signature")
require("philolo1.hop")
require("philolo1.luasnip")
require("philolo1.http")
require("philolo1.snippet")
